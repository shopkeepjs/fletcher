 ...
 */
export type CamelCaseString<S extends string> = S extends
  `${infer A}_${infer B}` ? `${Lowercase<A>}${Capitalize<CamelCaseString<B>>}`
  : Lowercase<S>;
 ...
 */
export type CamelToKebabCase<S extends string> = S extends
  `${infer T}${infer U}`
